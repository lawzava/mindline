...
asymmetric-agreement change not made in this revision (no-WASM stance vs ML-KEM). (5) An **active** adversary holding
...
- `g == g_current` with a *different* `gid`: a same-generation sibling
  ...
  same-`g` siblings to bloat retained keys and trial-decrypt.
...
endpoint-capture residual (4) above, but it is a larger change: it must be **hybrid ML-KEM-768 + X25519**, it pulls in a
vetted WASM PQC dependency against the repo's no-WASM stance, and it is
therefore a separate decision, never introduced silently.
