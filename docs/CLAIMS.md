...
| Forward secrecy | NOT-MADE until the §1.4 generation ratchet ships — the running code still has none; a leaked link exposes past and future wire content. The reviewed v3 design (PROTOCOL.md §1.4) delivers a *scoped* property only: relay-archived ciphertext of generations ≥1 stays unreadable against a passive operator given a later-leaked link. On implementation this becomes TRUE-WITH-CAVEATS with those exact exclusions named — link still grants entry + history-by-sync; at-rest stays under static `k_storage`; grants captured at an endpoint are link-readable; an active link-holder is a member. Must not be stated unqualified | Design + adversarial review in PROTOCOL.md §1.4/§2/§3.4; until shipped, stated plainly in PRIVACY.md "Honest caveats" and README "Honest limits" |
