...
| Members cannot forge, edit, or delete each other's messages or reactions | TRUE-WITH-CAVEATS — drafts/presence are unsigned and spoofable by room members (§2); history served during sync (deleted-state, reaction maps) is trusted from the serving member (§3.5/§3.7) | ECDSA P-256 envelope signatures (§2); ownership checks in `handlers.ts`; reaction membership keyed to the envelope-verified deviceId (`src/lib/p2p/reactions.ts`, `tests/unit/reactions.test.ts`) |
